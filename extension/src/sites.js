...
// Editors own their own query string — Docs rewrites ?tab=t.0 as you work, with
// no navigation behind it. Keying a page on `search` there made an ordinary
// click read as leaving the page, which cancelled the lesson and closed the
// panel under the learner. The document id is the identity that matters;
// moving to a different document still changes it. This lives here rather than
// in paint/navigation.js because it is knowledge about a site, and paint has
// none — its ownership check forbids the hostname outright.
const EDITOR_DOC = /^\/(document|spreadsheets|presentation|forms)\/d\/([^/]+)/;

/** Stable key for the page in the address bar: origin, path and query — or the editor document. */
export function pageKey(value = here()) {
  const url = new URL(value, here());
  const doc = url.hostname === LEGACY_SITE && url.pathname.match(EDITOR_DOC);
