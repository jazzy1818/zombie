...
Lessons are now scoped to the app they were written for: a lesson with no
`sites` field counts as a Google Docs lesson and is not offered anywhere else.
A loopback host is exempt, so the fixtures in this folder still see the whole
...
Generation follows the page: the panel sends its own URL, so a question asked
on GitHub explores GitHub rather than the configured demo doc. App-specific
selectors live in `pipeline/apps.js`, and a host with no entry there gets a
