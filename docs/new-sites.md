...
manifest already injects on every `http(s)` page. The library now uses each
lesson's `app` or explicit `sites` to offer it only on the relevant website.
...
Use the existing `app` field to identify the application. The schema (PLAN.md §5)
also accepts optional `sites` and `navigates` fields:

```jsonc
{
  "id": "github-new-issue",
  "app": "github",          // mapped to github.com; other hosts can use their hostname
  "sites": ["github.com"],   // optional override of the hosts inferred from app
...
**`app`** normally supplies the site: `google-docs`, `gmail` and `github` map to
their application hosts; an unregistered app can use its hostname directly.
Only a lesson with neither `app` nor `sites` falls back to legacy Google Docs.

**`sites`** overrides that mapping when needed. Accepted: `"github.com"`,
`["github.com"]`, a pasted URL, or `["*"]` for something genuinely
app-independent. A parent domain covers its subdomains (`google.com` matches
`docs.google.com`) but never the reverse.
