...
const freshBridgeState = () => ({ generateCalls: [], cancelCalls: [], pollCalls: [], playerCalls: [], playerPollCalls: [], matchCalls: [],
  mode: 'ok', delayMs: 60, match: 'unavailable', viewer: null, recordings: [], autoViewerReady: true, playerMode: 'ready', liveLoads: {}, livePlaybackPlans: {} });
...
(async () => {
  ...
  await context.route(`${bridgeUrl}/**`, async route => {
    ...
    const authoring = pathname === '/health' || pathname === '/generate' || pathname === '/match' || pathname.startsWith('/jobs/');
  ...
  await test('A confident local match never consults the bridge\'s model', async () => {
    bridgeState.match = 'none';
    await ask('how do I add a table of contents');
    await panel().locator('.bt-card-picker').waitFor();
    assert.equal(bridgeState.matchCalls.length, 0, 'a phrase match is decided locally');
    assert.deepEqual(await lessonLabels(), ['Add an automatic table of contents']);
  });
  ...
  await test("The bridge's model ruling a lesson out still leaves the near miss on offer", async () => {
    ...
    await panel().locator('.bt-card-picker').waitFor();
    assert.equal(bridgeState.matchCalls.length, 1, 'an unconfident question is checked with the bridge');
    assert.ok(bridgeState.matchCalls[0].candidates.some(c => c.id === 'styles-toc'), 'every lesson for this site is a candidate');
    ...
    assert.equal(labels.at(-1), 'None of these');
  });

  await test("The bridge's model can pick a lesson: offered alone, never launched unasked, with a way out on its preamble", async () => {
    bridgeState.match = 'pick:styles-toc';
    bridgeState.delayMs = 120000;
    await ask('make my document have chapters');
    await panel().locator('.bt-card-picker').waitFor();
    assert.deepEqual(await lessonLabels(), ['Add an automatic table of contents'], 'the model\'s pick is the one guess on offer');
    assert.equal(bridgeState.generateCalls.length, 0);
    await panel().locator('.bt-actions .bt-btn-lesson').first().click();
    await panel().locator('.bt-card-preamble').waitFor();
    const labels = await actionLabels();
    assert.ok(labels.includes("No, I'm not talking about this"), `expected the reject on the preamble, got ${JSON.stringify(labels)}`);
    await panel().getByRole('button', { name: "No, I'm not talking about this", exact: true }).click();
    await until(() => bridgeState.generateCalls.length === 1, 'refusing the picked lesson must send the question to be worked out');
    assert.equal(bridgeState.generateCalls[0].goal, 'make my document have chapters', 'the original question, not the lesson\'s title');
  });

  await test('An older bridge without /match falls back to the local shortlist', async () => {
    await ask('make my document have chapters');
    await panel().locator('.bt-card-picker').waitFor();
    assert.equal(bridgeState.matchCalls.length, 1);
    assert.ok((await lessonLabels()).includes('Add an automatic table of contents'));
  });
